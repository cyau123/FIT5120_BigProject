function show_job_recruiter_on_map( $atts ) {
	global $wpdb;
    $kinder_locations = $wpdb->get_results( "SELECT * FROM wp_job_center WHERE address != ''" );
                	...
                	<?php foreach ( $kinder_locations as $i => $location ) : ?>
                    	<div class="kinder-box">
                        	<li class="kinder-location" data-lat="<?php echo $location->latitude; ?>" data-lng="<?php echo $location->longitude; ?>">
                            	...
                            	<p style="font-size:16px;"><b>Phone:</b> <?php echo addslashes($location->contact); ?></p>
                        	</li>
                    	</div>
                	<?php endforeach; ?>
            	</ul>
    ...
    map.controls[google.maps.ControlPosition.TOP_CENTER].push(legend);
        ...
        <?php foreach ( $kinder_locations as $i => $location ) : ?>
            ...
            var marker = new google.maps.Marker({
				...
				icon: {
            		url: "https://img.icons8.com/ios-filled/50/null/marker-j.png"
        		}
            });
            ...
            (function (marker, infowindow, index, lat, lng) {
				...
				document.querySelector('.infowindow-search-route[data-index="' + index + '"]').addEventListener('click', function(e) {
					...
					if (!selectedDestination) {
						window.alert('Please select a Job Recruiter on map or from the list.');
						return;
					}
